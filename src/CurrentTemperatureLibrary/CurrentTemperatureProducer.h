...
extern PCSTR c_CurrentTemperatureIntrospectionXml;

ref class CurrentTemperatureProducer;

public interface class ICurrentTemperatureProducer
{
    event Windows::Foundation::TypedEventHandler<CurrentTemperatureProducer^, Windows::Devices::AllJoyn::AllJoynProducerStoppedEventArgs^>^ Stopped;
    event Windows::Foundation::TypedEventHandler<CurrentTemperatureProducer^, Windows::Devices::AllJoyn::AllJoynSessionLostEventArgs^>^ SessionLost;
    event Windows::Foundation::TypedEventHandler<CurrentTemperatureProducer^, Windows::Devices::AllJoyn::AllJoynSessionMemberAddedEventArgs^>^ SessionMemberAdded;
    event Windows::Foundation::TypedEventHandler<CurrentTemperatureProducer^, Windows::Devices::AllJoyn::AllJoynSessionMemberRemovedEventArgs^>^ SessionMemberRemoved;
};

public ref class CurrentTemperatureProducer sealed : [Windows::Foundation::Metadata::Default] ICurrentTemperatureProducer
{
public:
    CurrentTemperatureProducer(Windows::Devices::AllJoyn::AllJoynBusAttachment^ busAttachment);
    virtual ~CurrentTemperatureProducer();

    // The implementation of ICurrentTemperatureService that will handle method calls and property requests.
    property ICurrentTemperatureService^ Service
    {
        ICurrentTemperatureService^ get() { return m_serviceInterface; }
        void set(ICurrentTemperatureService^ value) { m_serviceInterface = value; }
    }
    ...
    virtual event Windows::Foundation::TypedEventHandler<CurrentTemperatureProducer^, Windows::Devices::AllJoyn::AllJoynProducerStoppedEventArgs^>^ Stopped;
    
    // This event will fire whenever the producer loses the session that it created.
    virtual event Windows::Foundation::TypedEventHandler<CurrentTemperatureProducer^, Windows::Devices::AllJoyn::AllJoynSessionLostEventArgs^>^ SessionLost;
    
    // This event will fire whenever a member joins the session.
    virtual event Windows::Foundation::TypedEventHandler<CurrentTemperatureProducer^, Windows::Devices::AllJoyn::AllJoynSessionMemberAddedEventArgs^>^ SessionMemberAdded;

    // This event will fire whenever a member leaves the session.
    virtual event Windows::Foundation::TypedEventHandler<CurrentTemperatureProducer^, Windows::Devices::AllJoyn::AllJoynSessionMemberRemovedEventArgs^>^ SessionMemberRemoved;

    // Send a signal to all members of the session to notify them that the value of Version has changed.
    void EmitVersionChanged();

    // Send a signal to all members of the session to notify them that the value of CurrentValue has changed.
...
private:
    static void CallSetLocationHandler(_Inout_ alljoyn_busobject busObject, _In_ alljoyn_message message);
    ...
    ICurrentTemperatureService^ m_serviceInterface;

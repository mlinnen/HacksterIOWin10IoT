...
CurrentTemperatureWatcher::CurrentTemperatureWatcher(AllJoynBusAttachment^ busAttachment) :
...
CurrentTemperatureWatcher::~CurrentTemperatureWatcher()
{
    UnregisterFromBus();
}

void CurrentTemperatureWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "net.protosystem.SmartSpaces.Environment.CurrentTemperature" };
...
void CurrentTemperatureWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "net.protosystem.SmartSpaces.Environment.CurrentTemperature"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "net.protosystem.SmartSpaces.Environment.CurrentTemperature"),
            port);
...
void CurrentTemperatureWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<CurrentTemperatureWatcher>
        };
...
void CurrentTemperatureWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_CurrentTemperatureIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &CurrentTemperatureWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void CurrentTemperatureWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void CurrentTemperatureWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
    AllJoynHelpers::DispatchEvent([=]() {

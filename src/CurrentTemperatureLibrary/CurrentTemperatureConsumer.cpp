...
std::map<alljoyn_interfacedescription, WeakReference*> CurrentTemperatureConsumer::SourceInterfaces;

CurrentTemperatureConsumer::CurrentTemperatureConsumer(AllJoynBusAttachment^ busAttachment)
...
CurrentTemperatureConsumer::~CurrentTemperatureConsumer()
{
...
void CurrentTemperatureConsumer::OnSessionLost(_In_ alljoyn_sessionid sessionId, _In_ alljoyn_sessionlostreason reason)
{
...
void CurrentTemperatureConsumer::OnSessionMemberAdded(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
void CurrentTemperatureConsumer::OnSessionMemberRemoved(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
QStatus CurrentTemperatureConsumer::AddSignalHandler(_In_ alljoyn_busattachment busAttachment, _In_ alljoyn_interfacedescription interfaceDescription, _In_ PCSTR methodName, _In_ alljoyn_messagereceiver_signalhandler_ptr handler)
{
...
IAsyncOperation<CurrentTemperatureJoinSessionResult^>^ CurrentTemperatureConsumer::JoinSessionAsync(
    _In_ AllJoynServiceInfo^ serviceInfo, _Inout_ CurrentTemperatureWatcher^ watcher)
{
    return create_async([serviceInfo, watcher]() -> CurrentTemperatureJoinSessionResult^
    {
        auto result = ref new CurrentTemperatureJoinSessionResult();
        result->Status = AllJoynStatus::Ok;
        result->Consumer = nullptr;

        result->Consumer = ref new CurrentTemperatureConsumer(watcher->BusAttachment);
...
IAsyncOperation<CurrentTemperatureSetLocationResult^>^ CurrentTemperatureConsumer::SetLocationAsync(_In_ Platform::String^ interfaceMemberLocation)
{
    return create_async([this, interfaceMemberLocation]() -> CurrentTemperatureSetLocationResult^
    {
        ...
        TypeConversionHelpers::SetAllJoynMessageArg(alljoyn_msgarg_array_element(inputs, 0), "s", interfaceMemberLocation);
...
IAsyncOperation<CurrentTemperatureGetVersionResult^>^ CurrentTemperatureConsumer::GetVersionAsync()
{
    return create_async([this]() -> CurrentTemperatureGetVersionResult^
    {
...
IAsyncOperation<CurrentTemperatureGetCurrentValueResult^>^ CurrentTemperatureConsumer::GetCurrentValueAsync()
{
    return create_async([this]() -> CurrentTemperatureGetCurrentValueResult^
    {
        ...
        result->Status = getContext.GetStatus();
        result->CurrentValue = getContext.GetValue();
        return result;
    });
}

IAsyncOperation<CurrentTemperatureGetLocationResult^>^ CurrentTemperatureConsumer::GetLocationAsync()
{
    return create_async([this]() -> CurrentTemperatureGetLocationResult^
    {
...
void CurrentTemperatureConsumer::OnPropertyChanged(_In_ alljoyn_proxybusobject obj, _In_ PCSTR interfaceName, _In_ const alljoyn_msgarg changed, _In_ const alljoyn_msgarg invalidated)
{
    UNREFERENCED_PARAMETER(obj);
    ...
    for (size_t i = 0; i < changedPropertyCount; i++)
    {
        ...
        if (strcmp("Version", propertyName) == 0)
        {
            VersionChanged(this, nullptr);
        }
        if (strcmp("CurrentValue", propertyName) == 0)
        {
            CurrentValueChanged(this, nullptr);
        }
    }
}

int32 CurrentTemperatureConsumer::JoinSession(_In_ AllJoynServiceInfo^ serviceInfo)
{
    alljoyn_sessionlistener_callbacks callbacks =
    {
        AllJoynHelpers::SessionLostHandler<CurrentTemperatureConsumer>,
        AllJoynHelpers::SessionMemberAddedHandler<CurrentTemperatureConsumer>,
        AllJoynHelpers::SessionMemberRemovedHandler<CurrentTemperatureConsumer>
    };
    ...
    PCSTR propertyNames[] = { "Version", "CurrentValue" };
    ...
    RETURN_IF_QSTATUS_ERROR(alljoyn_proxybusobject_registerpropertieschangedlistener(
        ...
        AllJoynHelpers::PropertyChangedHandler<CurrentTemperatureConsumer>, 
        m_weak));


    alljoyn_interfacedescription description = alljoyn_busattachment_getinterface(m_nativeBusAttachment, "net.protosystem.SmartSpaces.Environment.CurrentTemperature");
